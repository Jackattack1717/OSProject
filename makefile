...
bootdisk: bootloader kernel
	...
	dd conv=notrunc if=$(KERNEL) of=$(DISK_IMG) bs=512 count=1 seek=1 #writes kernel to floppy no filesystem
	#mcopy -i $(DISK_IMG) $(KERNEL) "::kernel.bin" #copies kernel to the diskimg via DOS command
